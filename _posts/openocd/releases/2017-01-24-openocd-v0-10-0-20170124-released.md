...
One important note: the old `ft2232` interface was deprecated and all configuration files using it were removed from the original OpenOCD distribution. As a consequence, JTAG probes that used the `ft2232` interface (like Olimex ARM-USB-OCD) are no longer supported.
